...
[q_errors5, r_adjs5, z_adjs5], name5 = pickle_load(description5)
...
# FOR SUB-INDEX
def animate(axs, tt, p=0.1):
    ylim = axs.get_ylim()
    xlim = axs.get_xlim()
    axs.text(xlim[0]+0.5*(xlim[1]-xlim[0]), ylim[0] - p * (ylim[1] - ylim[0]), "({})".format(tt),
...
axes[0, 0].set(xlabel="Latent Variable $Z_1$", ylabel="Latent Variable $Z_2$")
axes[0, 0].axis('equal')
animate(axes[0, 0], 'a')
animate_name(axes[0, 0], 'Gaussian Mixture')

axes[0, 1].scatter(z_adjs2[:, 0], z_adjs2[:, 1], **params)
axes[0, 1].set(xlabel="Latent Variable $Z_1$", ylabel="Latent Variable $Z_2$")
axes[0, 1].axis('equal')
animate(axes[0, 1], 'b', 0.32)
animate_name(axes[0, 1], 'Swiss Roll')


axes[0, 2].scatter(z_adjs3[:, 0], z_adjs3[:, 1], **params)
axes[0, 2].set(xlabel="Latent Variable $Z_1$", ylabel="Latent Variable $Z_2$")
axes[0, 2].axis('equal')
animate(axes[0, 2], 'c')
...
axes[1, 0].axis('equal')
axes[1, 0].set(xlabel="Latent Variable $Z_1$", ylabel="Latent Variable $Z_2$")
animate(axes[1, 0], 'd')
animate_name(axes[1, 0], 'Standard Uniform')


axes[1, 1].scatter(z_adjs5[:, 0], z_adjs5[:, 1], **params)
axes[1, 1].axis('equal')
axes[1, 1].set(xlabel="Latent Variable $Z_1$", ylabel="Latent Variable $Z_2$")
animate(axes[1, 1], 'e', 0.32)
animate_name(axes[1, 1], 'Gaussian')

axes[1, 2].remove()
fig.tight_layout(pad=1, w_pad=2, h_pad=2.5)
# C:\Users\CYD\Desktop\IAI_Conv\supervised
plt.savefig('{}/{}.png'.format('./historical_records', "Latent_Space"))
plt.show()
