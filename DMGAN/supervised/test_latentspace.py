...
# description = 'hybrid_GAN_cc_res-Dz=0.01-R=1-Lat=1.5-Tv=0.01-d-gs-bc-gs-2018-02-04'
checkpoint_path = '/check_point/{}'.format(description)
...
def main():
    ...
    with tf.Session(
        ...
        pickle_save([q_errors, r_adjs, z_adjs, z_true], ["productions", "adjustment", "latent_variables"],
                    '{}/{}-metric_plus.pkl'.format(config.logs_path, description))
...
if __name__ == "__main__":
    ...
    vis.jointplot(z_true[:, 0], z_true[:, 1], config.latent_path, name='Scatter_latent')
    #
