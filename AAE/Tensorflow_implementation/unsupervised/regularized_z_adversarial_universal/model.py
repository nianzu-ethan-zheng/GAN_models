...
def build_graph(is_test=False):
    ...
    images = tf.placeholder(
        dtype=tf.float32,
        shape=[None, config.ndim_x]
    )
    z_prior = tf.placeholder(
        dtype=tf.float32,
        shape=[None, config.ndim_noise]
    )
    z_sampler = tf.placeholder(
        dtype=tf.float32,
        shape=[None, config.ndim_z]
    )
    ...
    with tf.variable_scope('encoder'):
        z_representation = encoder_x_z(images, z_prior)
    with tf.variable_scope('decoder'):
        reconstruction = decoder_z_x(z_representation)

    if is_test:
        test_handle = Object()
        test_handle.z_prior = z_prior
    ...
    # 0 -> true sample
    # 1 -> generated sample
    loss_discriminator = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=probability_true_sample, labels=tf.ones_like(probability_true_sample)) + \
                              tf.nn.sigmoid_cross_entropy_with_logits(logits=probability_fake_sample, labels=tf.zeros_like(probability_true_sample)))

    loss_encoder = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=probability_fake_sample, labels=tf.ones_like(probability_fake_sample)))
